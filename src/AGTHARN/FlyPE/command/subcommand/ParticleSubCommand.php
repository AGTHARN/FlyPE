...
class ParticleSubCommand extends BaseSubCommand
{
    ...
    public function __construct(Main $plugin, string $name, string $description, $aliases = [])
    {
        ...
        $this->particleForm = $plugin->particleForm;
    ...
    {
        $this->setPermission('flype.command.particle');
    }
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        if (!$this->plugin->config->getConfig('cosmetic', 'enable-particle')) {
            $senderSession->sendTranslated('flype.particle.allowed.false');
            return;
        }
        $this->particleForm->openParticleForm($sender);
    }
}
