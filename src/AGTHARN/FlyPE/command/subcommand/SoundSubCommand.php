...
class SoundSubCommand extends BaseSubCommand
{
    ...
    public function __construct(Main $plugin, string $name, string $description, $aliases = [])
    {
        ...
        $this->soundForm = $plugin->soundForm;
    ...
    {
        $this->setPermission('flype.command.sound');
    }
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        if (!$this->plugin->config->getConfig('cosmetic', 'enable-sound')) {
            $senderSession->sendTranslated('flype.sound.allowed.false');
            return;
        }
        $this->soundForm->openSoundForm($sender);
    }
}
